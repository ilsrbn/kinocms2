...
export default {
  ...
  actions: {
    async fetchCards(ctx) {
      try {
        let cards = []
        const snapshot = await db.ref('/banners/main').once('value')
    ...
    async saveCards(ctx) {
      ...
      if (ctx.state.picturesToWrite.length != 0) {
        try {
          ...
          for (var q = ctx.state.picturesToWrite.length - 1; q >= 0; q--) {
            ...
            console.log(fl)
            const picRef = storage.ref('/banners/main/' + fl.picture.name)
      ...
      return await db.ref('/banners/main').set(ctx.state.cards)
    }
  },
