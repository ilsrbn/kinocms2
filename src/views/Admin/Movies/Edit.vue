...
export default {
  ...
  computed: {
    movies() {
      return { ru: this.moviesFull.ru, ua: this.moviesFull.ua };
    },
  ...
  async mounted() {
    ...
    if ((await fetch(this.path)) != null) {
      this.moviesFull = await fetch(this.path);
    } else {
  ...
  methods: {
    bannerPreview(lang, event) {
      ...
      if (
        this.moviesFull[lang].banner != "" &&
        this.moviesFull[lang].banner[0] != "b"
      ) {
        this.bannerRemove[lang] = this.moviesFull[lang].banner;
      }

      this.$set(this.moviesFull[lang], "banner", URL.createObjectURL(file));
      this.bannerWrite[lang] = file;
    },

    bannerClear(lang) {
      if (this.moviesFull[lang].banner[0] != "b") {
        this.bannerRemove[lang] = this.moviesFull[lang].banner;
      }
      this.$set(this.moviesFull[lang], "banner", "");
    },

    galleryPreview(index, lang, event) {
      const file = event.target.files[0];
      if (
        this.moviesFull[lang].gallery[index] != "" &&
        this.moviesFull[lang].gallery[index][0] != "b"
      ) {
        this.galleryRemove[lang][index] = this.moviesFull[lang].gallery[index];
      }
      this.$set(
        this.moviesFull[lang].gallery,
        index,
    ...
    galleryClear(index, lang) {
      if (this.moviesFull[lang].gallery[index][0] != "b") {
        this.galleryRemove[lang][index] = this.moviesFull[lang].gallery[index];
      }
      this.$set(this.moviesFull[lang].gallery, index, "");
    },
    ...
    async save() {
      ...
      await save(`/movies/${this.$route.params.id}`, this.moviesFull);
      this.saved = true;
      this.loadingState = "";
    ...
    async writeBannerToStorage() {
      for (const lang in this.bannerWrite) {
        ...
        if (el != "") {
          try {
            this.moviesFull[lang].banner = await addToStorage(
              el,
              "movies",
    ...
    async writeGalleryToStorage() {
      for (const language in this.galleryWrite) {
        for (let i = this.galleryWrite[language].length - 1; i >= 0; i--) {
          ...
          if (el != "") {
            try {
              this.moviesFull[language].gallery[i] = await addToStorage(
                el,
                "movies",
                this.$route.params.id
              );
