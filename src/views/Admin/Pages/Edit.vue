...
export default {
  ...
  data () {
    return {
      ...
      uploading: false,
      success: false,
      path: `/pages/${this.$route.params.id}`,
  ...
  methods: {
    bannerPreview(lang, event) {
      ...
      if (this.pagesFull[lang].banner != '' && this.pagesFull[lang].banner[0] != 'b') {
        this.bannerRemove[lang] = this.pagesFull[lang].banner
      }

      this.$set(this.pagesFull[lang], 'banner', URL.createObjectURL(file))
      this.bannerWrite[lang] = file
    },

    bannerClear(lang) {
      if (this.pagesFull[lang].banner[0] != 'b') {
        this.bannerRemove[lang] = this.pagesFull[lang].banner
      }
      this.$set(this.pagesFull[lang], 'banner', '')
    },

    galleryPreview(index, lang, event) {
      const file = event.target.files[0]
      if (this.pagesFull[lang].gallery[index] != '' && this.pagesFull[lang].gallery[index][0] != 'b') {
        this.galleryRemove[lang][index] = this.pagesFull[lang].gallery[index]
      }
      this.$set(this.pagesFull[lang].gallery, index, URL.createObjectURL(file))
      this.galleryWrite[lang][index] = file
    },

    galleryClear(index, lang) {
      if (this.pagesFull[lang].gallery[index][0] != 'b') {
        this.galleryRemove[lang][index] = this.pagesFull[lang].gallery[index]
      }
      this.$set(this.pagesFull[lang].gallery, index, '')
    },
    ...
    async save() {
      ...
      await save(`/pages/${this.$route.params.id}`, this.pagesFull)
      this.saved = true
      this.loadingState = ''
    ...
    async writeBannerToStorage() {
      for (const lang in this.bannerWrite) {
        ...
        if (el != '') {
          try {
            this.pagesFull[lang].banner = await addToStorage(el, 'pages', this.$route.params.id)
            this.loadingLeft += 1
    ...
    async writeGalleryToStorage() {
      for (const language in this.galleryWrite) {
        for (let i = this.galleryWrite[language].length - 1; i >= 0; i--){
          ...
          if(el != '') {
            try {
              this.pagesFull[language].gallery[i] = await addToStorage(el, 'pages', this.$route.params.id)
              this.loadingLeft += 1
              console.log('[SUCCESS]\n', el, 'writen to storage')
            } catch(e) {
              console.log('Error while writing picture', el, 'into storage.', e)
